...
(function(mod) {
  ...
  function isSimpleAnnotation(spec) {
    ...
    for (var prop in spec)
      if (prop != "!type" && prop != "!doc" && prop != "!url" && prop != "!span" && prop != "!data" && prop != "!help")
        return false;
    return true;
  }
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      if (typeof inner == "string") {
        ...
        var doc = inner["!doc"], url = inner["!url"], span = inner["!span"], help = inner["!help"];
        if (doc) {
        ...
        if (help) {
            if (type && type instanceof infer.Obj) type.help = help;
            known.help = help;
        }
