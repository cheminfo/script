...
$(function() {
	...
	loadHistory = function() {
		...
		commandServer(getDataDirCommand($currentProjectNode.getKeyPath() + "/"
				+ addresses().historyPath), function(a) {
			if (!a.error && a.result) {
	...
	function displayHistoryDiv(aF) {
		...
		var $noHistory = $('<div>').html("No history available").addClass(
				"no-history");
		...
		$.each(aF, function(aName, blocks) {
			...
			var title = aName;
			if (c == 1)
				title += " <small>(current file)</small>";
	...
	function getFileBlock(file) {
		...
		// var path = $currentProjectNode.getKeyPath() + "/" +
		// addresses().historyPath + "/" + file.name ;

		var $diffDiv = $('<span>').append($('<span>').html("Diff:"));
		var linkSource = $('<a>', {
			'href' : '#'
		}).html("file 'from'").on("click", function() {
			sourceDiff = file;
		...
		var linkDest = $('<a>', {
			'href' : '#'
		}).html("file 'to'").on("click", function() {
		...
		var linkEdit = $('<a>', {
			'href' : '#'
		}).html("Load").addClass("btn btn-small");
			...
			commandServer(getDataLoadFile(path), function(a, b) {
				if (!a.error) {
					...
					var loadOldScriptToCM = function() {
						...
						$writeConsole("Script <i>" + realName
								+ "</i> loaded from history.");
						$setCMChanged();
						// tryToLoadLastResult();
					...
					if ($isCMSaved)
						...
						$confirm("Current script was not saved. Erase it?",
								loadOldScriptToCM);
				}
			});
		});
		...
		$li.append(" ").append(
				$('<span>').html(new Date(file.lastModified).toLocaleString())
						.addClass("history-date"));
	...
	function showDiff() {
		...
		if (!sourceDiff) {
			isOkay = false;
			$blockInfo.append($('<li>').html(
					"Choose a file 'from' for the difference"));
		}

		if (!destinationDiff) {
			isOkay = false;
			$blockInfo.append($('<li>').html(
					"Choose a file 'to' for the difference"));
		}
	...
	function getAdd(file) {
		return $currentProjectNode.getKeyPath() + "/" + addresses().historyPath
				+ "/" + file.name;
	}
	...
	function getCompTitle(s) {
		...
		return $('<span>').append(
		// $('<b>').html(t[1])
		$('<b>').html(s.name.split(".")[1])).append(" ").append(
	...
	function displayDiffTitle() {
		var t = $("<span>").append("From ").append(getCompTitle(sourceDiff))
				.append(" to ").append(getCompTitle(destinationDiff))
		$blockDiff.prepend(t);
	}
	...
	function displayDiff(f1, f2) {
		...
		if (f1.content && f2.content) {
			...
			$blockDiff
					.html($('<pre>').html(
							dmp.diff_prettyHtml(dmp.diff_main((f1.content
									.replace(/\n/g, "\r")), (f2.content)
									.replace(/\n/g, "\r")))));
			displayDiffTitle();
		}
